...
try {
  ...
  assert(pdfAssets.includes('function authorizeManagedPdfAssetDeletion'), 'PDF cleanup should centralize managed-delete authorization')
  assert(pdfAssets.includes('function isLinkedExternalPdfAsset'), 'PDF cleanup must detect linked external originals')
  assert(pdfCleanupBody.includes('if (!linkedExternal)'), 'PDF cleanup must skip linked external file_path delete candidates')
  assert(pdfCleanupBody.includes('authorizeManagedPdfAssetDeletion(docId'), 'PDF cleanup should authorize every delete candidate')
  ...
  assert((pdfAssets.slice(0, pdfCleanupEnd).match(/inspectManagedDeleteTarget/g) || []).length >= 1, 'PDF cleanup should use managed path boundary')
  assert(pdfCleanupBody.includes("kind: 'document-asset'") || pdfAssets.includes("kind: 'document-asset'"), 'PDF asset cleanup should use the managed asset boundary')
