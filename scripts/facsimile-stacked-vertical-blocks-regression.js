...
assertIncludes('function shouldRenderAsTableBlock(', 'facsimile proofreader should distinguish true tables from vertical pseudo tables')
...
assertIncludes('&& isLikelyVerticalPseudoTableBlock(block, tableProjection)', 'facsimile persistence should downgrade vertical pseudo tables in vertical page mode using the shared canonical projection')
...
assertIncludes('const isTable = shouldRenderAsTableBlock(block, pageVerticalMode, tableProjection)', 'facsimile renderer should not draw vertical pseudo tables as HTML tables')
...
assertIncludes("if (hasVerticalColumnTextShape(block, resolvedTableProjection)) return 'vertical'", 'mixed vertical pages should preserve detected vertical column text before horizontal heuristics')
assertIncludes("if (isOrdinaryVerticalPageTextBlock(block, resolvedTableProjection)) return 'vertical'", 'dominant vertical pages should coerce ordinary body text blocks to vertical before strong horizontal heuristics')
...
assertDocumentViewIncludes('() => getOcrCoordinateSourceSize(proofingOcrResultObj || ocrResultObj)', 'original image overlay should use the proofing OCR result coordinate basis')
  ...
  assertDocumentViewIncludes('const editableFacsimileOcrResult = useMemo<FacsimileOcrResult>', 'facsimile editor should receive an empty editable payload when OCR is missing')
  assertDocumentViewIncludes('const canUseManualFacsimileLayout = facsimileProofCandidate', 'image-backed pages without OCR should still expose manual facsimile editing')
  assertIncludes('data-manual-layout-empty-state', 'empty pages must render an editable manual-layout canvas instead of an early empty return')
  assertNotIncludes('if (blocks.length === 0 && !pageImageSrc) return <Empty', 'empty pages must not be blocked from manual block creation')
