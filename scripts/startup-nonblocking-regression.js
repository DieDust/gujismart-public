...
assert(!mainIndex.includes('scheduleStartupMetadataReclassification'), 'Startup must not silently submit legacy documents to paid AI reclassification.')
...
assert(
  ocrIpc.includes('function shouldPersistBatchOcrForRecovery')
    ...
    && ocrIpc.includes('createRecoverableBatchOcrItems(persistedChunk, documentConcurrency)')
    && !ocrIpc.includes('transactionAsync(() => undefined')
    && ocrIpc.includes('runBoundedDocumentWorkers')
