...
assert.ok(proofreader.includes('updateBlockWithHistory(editingBlockId,') && proofreader.includes('manualLayoutDraft.updateBlock(blockId, changes)'), 'text and table editor changes must enter the revisioned draft and undo history before debounce')
...
assert.ok(documentView.includes('const handleSavePage = async (pageId: string, data: PageUpdatePayload, echoToken?: string): Promise<boolean>'), 'the shared page save path must report its real success outcome without rejecting legacy fire-and-forget callers')
...
assert.ok(textEditor.includes('onSave: (pageId: string, data: PageUpdatePayload, echoToken?: string) => Promise<boolean>'), 'the text editor callback must expose the real async database outcome')
