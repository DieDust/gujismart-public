...
assertIncludes(app, 'function normalizeDefaultTabGroupTitles(', 'default tab group titles should be normalized after restore and membership changes')
assertIncludes(app, 'getTabGroupIdsInTabOrder(tabGroups, tabs).forEach((groupId) => {', 'default tab group numbering should follow the visible tab strip order')
assertIncludes(app, 'protectedGroupIds.has(group.id)', 'closed-history groups should not be renamed while they are restorable')
assertIncludes(app, 'const protectedGroupIds = new Set(', 'App should protect restorable closed groups during title normalization')
