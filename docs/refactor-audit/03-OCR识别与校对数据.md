...
## 14. 2026-07-17 超大 PDF 与横向表格 OCR 修复

- 异步 PDF OCR 不再为整本文献预生成页图。200 MB、数百至上千页的 PDF 会直接进入上传/分片流程，只有服务结果异常或失败的页才按需渲染页图并单页补跑，避免进度长期停在 `0/总页数`。
- 异步结果仍按服务分片逐批后处理和保存；保存批次收紧为 8 页、后处理批次为 16 页，每批之间主动让出主进程事件循环。整书 OCR 结构重建也改为 8 页分段读取、压缩和写入，降低保存阶段“未响应”的连续阻塞时间。
- 修正质量门禁的适用范围：异步 PDF 的“疑似误判表格”规则不再套用到真实单页图片 OCR，横向统计表不会仅因返回 table block 而被拒绝写入。
- 单页 OCR 下拉菜单新增“横向 OCR 本页（顺时针旋转）”。页图仅在临时文件中旋转后上传；识别完成后 layout、word 和页面尺寸坐标映射回原始页，并删除基于临时方向生成的旧 IR，强制按原页坐标重建。
- 失败补跑仍只处理 pending/error 页，不重做已成功页，也不会清空人工校对内容；未增加数据库迁移和第三方运行时依赖。

自动验收覆盖 `check:ocr-upload`、`check:ocr-result-save`、`check:ocr-coordinate`、`check:ocr-ir`、`check:ocr-benchmark` 和完整 `npm run check`。联网飞桨 OCR 未在自动验收中调用，避免消耗用户额度或上传真实文献。
