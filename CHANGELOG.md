...
- 备份恢复与 Electron 下载链路改用官方维护的安全解压分支，修复旧 `extract-zip` 依赖的符号链接路径穿越风险；不改变现有 ZIP 备份格式与旧备份兼容性。
...
- Backup restore and Electron download tooling now use the officially maintained safe extraction fork, removing the symlink path-traversal exposure in the old `extract-zip` dependency without changing the ZIP backup format or legacy-backup compatibility.
